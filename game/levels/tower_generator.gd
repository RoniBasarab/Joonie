...
var coin_scene: PackedScene = preload("res://game/levels/level_one/coin.tscn")
...
func _on_generator_timer_timeout() -> void:
	...
	#WALL GENERATION
	start_wall_spawn_point = Vector2i(left_tower_wall_point.position.x - 16, left_tower_wall_point.position.y - 8)
	...
	if platform_random_position_x <= (30 - random_pattern_size):
		...
		get_tree().get_first_node_in_group("coins").add_child(coin_scene_instance)
		coin_scene_instance.global_position = Vector2(randi_range(platform_random_position_x, platform_random_position_x + random_pattern_size), platforms_spawn_point.position.y - 1) * 16
